...
(*
val xs = argsof(d0cs) : List0(string)
val () = println!("schema(d0cs) = ", xs) //argsof(d0cs))

val () = assertloc(list_length(d0cs) >= 1)
val x1 = list_get_at(d0cs, 0)
val () = (
...
(* val _ = $showtype(x1) *)
(*
val xs0 = argsof_d0ecl<>(x1)
val () = println!("schema(d0cs) = ", xs0) //argsof(d0cs))
*)
